...
/*
SetCursorPosition is a custom function that can move the cursor to a different location on the screen
...
void GenerateBullets() {
	Bullets[BulletsIndex][0] = 23;
	Bullets[BulletsIndex][1] = ShipPosition;
	BulletsIndex++;
...
int HitEnemy() {
	for (int i = 0; i < 10; i++) {
		for (int j = 0; j < 4; j += 2) {
			if (Bullets[i][j] != 0) {
				if (Bullets[i][j] >= EnemyVertical[0] && Bullets[i][j] <= EnemyVertical[0] + 1) {
					if (Bullets[i][j + 1] >= EnemyHorizontal[0] && Bullets[i][j + 1] <= EnemyHorizontal[0] + 1) {
...
void play() {
	...
	EnemyVertical[0] = EnemyVertical[1] = 4;
	...
	SetCursorPosition(27, 8); cout << "PRESS ANY KEY TO START";
	_getch();
	SetCursorPosition(27, 8); cout << "                      ";
